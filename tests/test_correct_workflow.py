...
def test_main(args):
    ...
    check_dependencies()
    validate_input_files(args.read_paths)
    validate_input_files([args.reference_sketch_path])
    validate_input_files([args.clade_config_path])

    for read_path in args.read_paths:
        assert is_fastq(read_path)
    ...
    testsample = AuriClassAnalysis(
        ...
        genome_size_range=args.genome_size_range,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Run mash screen
        testsample.run_mash()
        assert testsample.mash_output.to_dict() == mash_output_to_dict
    ...
    assert testsample.qc_distance == None
